...
int two,three,four,five,six,total=0;
...
int aHash(Mat matSrc1, Mat matSrc2)//返回的值越小越相似
{
	...
	unsigned short iAvg1 = 0, iAvg2 = 0;
    unsigned short arr1[64], arr2[64];
...
/*
		for (int j = 0; j < 8; j+=4)
		{
			float* data1 = matDst1.ptr<float>(i);
			float* data2 = matDst2.ptr<float>(i);
			//int tmp1 = tmp + j;

	
  			//uint32x4 v0;
			float32x4_t v0,v1;
			v0 = vld1q_f32(&data1[j]);
			v1 = vdupq_n_f32(16);	//将常数16 存入v1
			v1 = vrecpsq_f32(v1);   //取16的倒数以便进行乘法
			//v2 = vmulq_f32(vrecpsq_f32(v1,v2),v2);
			//v2 = vmulq_f32(vrecpsq_f32(v1,v2),v2);
			v0 = vmulq_f32(v0, v1);  //data1[j] 乘 16的倒数
			vst1q_f32(&arr11[j], v0);     //将计算的值回写到arr11数组中
			
			//uint32x4 v3;
			float32x4_t v3,v4;
			v3 = vld1q_f32(&data2[j]);
			v4 = vdupq_n_f32(16);	//将常数16 存入v4
			v4 = vrecpsq_f32(v4);   //取16的倒数以便进行乘法
			//v5 = vmulq_f32(vrecpsq_f32(v4,v5),v5);
			//v5 = vmulq_f32(vrecpsq_f32(v4,v5),v5);
			v3 = vmulq_f32(v3, v4);  //data1[j] 乘 16的倒数
			vst1q_f32(&arr22[j], v3);    //将计算的值回写到arr22数组中
			cout << arr11[j];
			cout << arr22[j] << endl;
...
*/
	...
	if((double)(endTime - startTime) / CLOCKS_PER_SEC==2e-06 && total <= 200){
		two+=1;
}
	if((double)(endTime - startTime) / CLOCKS_PER_SEC ==3e-6 && total <= 200){
		three+=1;
}
	if((double)(endTime - startTime) / CLOCKS_PER_SEC ==4e-6 && total <= 200){
		four+=1;
}
	if((double)(endTime - startTime) / CLOCKS_PER_SEC ==5e-6 && total <= 200){
		five+=1;
}
	if((double)(endTime - startTime) / CLOCKS_PER_SEC ==6e-6 && total <= 200){
		six+=1;
}
	if(total == 200){
cout << "2 times is "<<two << endl;
cout << "3 times is "<<three << endl;
cout << "4 times is "<<four << endl;
cout << "5 times is "<<five << endl;
cout << "6 times is "<<six << endl;
}
